...
2.  **生成規格與計畫 (Spec & Plan)**
    ...
    *關於目錄命名邏輯（如中文支援、內容決定方式）與產出檔案狀態（擬稿機制），請參考 [Q&A Q7 與 Q8](#q7)。*
...
1.  **開始編碼**
    ...
    /conductor:implement
    ```
    AI 會依照 `plan.md` 的順序撰寫程式碼，並支援 TDD 模式（先寫測試，再寫實作）。
...
2.  **自動化審查 (Automated Review)**
    ...
    /conductor:review
    ```
    AI 會比對產出的程式碼是否符合 `spec.md` 與全域的 `conventions.md`。
...
若您的儲存庫有多個專案，請建立 `.conductorignore` 檔案以防止 AI 產生幻覺：
